...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    y_hat = softmax(np.matmul(outsideVectors, centerWordVec)) # (V, N) * (N, ) = (V, )
    loss = -np.log(y_hat[outsideWordIdx])

    y = np.zeros_like(y_hat)
    y[outsideWordIdx] = 1

    gradCenterVec = np.inner(np.transpose(outsideVectors), y_hat - y) # (N, V) * (V, ) = (N, )
    gradOutsideVecs = np.outer(y_hat - y, centerWordVec) # (V, ) * (N, )^T = (V, N)
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    u_o = outsideVectors[outsideWordIdx] # (N, )
    u_K = outsideVectors[negSampleWordIndices] # (K, N)
    pos = sigmoid(np.inner(u_o, centerWordVec)) # (1, ))
    neg = sigmoid(np.inner(-u_K, centerWordVec)) #(K, ))
    loss = -np.log(pos) - np.sum(np.log(neg)) # (1, )

    gradCenterVec = -(1 - pos) * u_o + np.matmul(1 - neg, u_K)

    gradOutsideVecs = np.zeros_like(outsideVectors)
    unique, index, count = np.unique(negSampleWordIndices, return_index=True, return_counts=True)
    gradOutsideVecs[unique] = np.outer(count * (1 - neg[index]), centerWordVec)
    gradOutsideVecs[outsideWordIdx] = -(1 - pos) * centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    centerWordIdx = word2Ind[currentCenterWord] # center word index
    centerWordVec = centerWordVectors[centerWordIdx] # center word vector
    for o_w in outsideWords:
        o_idx = word2Ind[o_w]
        l, dv_c, dU = word2vecLossAndGradient(centerWordVec, o_idx, 
